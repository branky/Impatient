...
public class Main {
	public static void main(String[] args) {
		...
		if (System.getProperty("os.name").toLowerCase().indexOf("win") >= 0) {
			// Windows
			properties.put("fs.file.impl", "com.conga.services.hadoop.patch.HADOOP_7682.WinLocalFileSystem");
		}
		...
		docPipe = new Each(docPipe, scrubArguments, new ScrubFunction(scrubArguments),
				Fields.RESULTS);
		...
		FlowDef flowDef = FlowDef.flowDef().setName("tfidf").addSource(docPipe, docTap).addSource(
				stopPipe, stopTap).addTailSink(tfidfPipe, tfidfTap).addTailSink(wcPipe, wcTap).addTrap(
				docPipe, trapTap).addCheckpoint(idfCheck, checkTap);
